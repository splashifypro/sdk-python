...
with splashifypro.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = splashifypro.MessagesApi(api_client)
    body = None # object | { message_id: string (our message_id of the inbound message; a WhatsApp wamid also works), to?: string (the contact's number; helps find an older message) }
